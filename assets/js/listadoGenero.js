...
function generarPDF() {
    var datosFiltradosOrdenados = tablaGenero.rows({ search: 'applied', order: 'applied' }).data().toArray();
    ...
    var form = $('<form action="../pages/fpdf/listadoGenero.php" method="post" target="_blank">');
    var input = $('<input type="hidden" name="data">').val(jsonData);
    form.append(input);
  
    // Agregar el formulario a la página y enviarlo
    form.appendTo('body').submit();
    EventoBitacora(1, 14, "Imprimió el reporte de LISTADO DE GENERO");//bitacora reporte
  }
  
  //botón en la página para generar y descargar el PDF
  $('#btn_descargar_reporteGenero_pdf').on('click', function() {
    generarPDF();
  });
...
function eventoCerrarModal(){
  ...
  if($('#descripcion_Genero').val() != ""){ //validad que los input hayan datos
