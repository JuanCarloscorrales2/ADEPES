...
function generarPDF() {
    var datosFiltradosOrdenados = tablaTipopersona.rows({ search: 'applied', order: 'applied' }).data().toArray();
    ...
    var form = $('<form action="../pages/fpdf/listadoTipoPersona.php" method="post" target="_blank">');
  ...
  $('#btn_descargar_rTipoPersona_pdf').on('click', function() {
    generarPDF();
  });
...
function eventoCerrarModal(){
  ...
  if($('#descripcion_Tipopersona').val() != ""){ //validad que los input hayan datos
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          $('#registral_tipo_persona').modal('show');
        }
      });
  }
 
