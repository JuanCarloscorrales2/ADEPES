...
function generarPDF() {
    var datosFiltradosOrdenados = tablaCreditoaval.rows({ search: 'applied', order: 'applied' }).data().toArray();
    ...
    var form = $('<form action="../pages/fpdf/listadoCreditoAval.php" method="post" target="_blank">');
  ...
  $('#btn_descargar_rCredAval_pdf').on('click', function() {
    generarPDF();
  });
...
function eventoCerrarModal(){
  ...
  if($('#descripcion_credito_aval').val() != ""){ //validad que los input hayan datos
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          $('#registral_creditoaval').modal('show');
        }
      });
  }
 
