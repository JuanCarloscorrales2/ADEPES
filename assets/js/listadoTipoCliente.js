...
function generarPDF() {
    var datosFiltradosOrdenados = tablaTipocliente.rows({ search: 'applied', order: 'applied' }).data().toArray();
    ...
    var form = $('<form action="../pages/fpdf/listadoTipoCliente.php" method="post" target="_blank">');
  ...
  $('#btn_descargar_rTipoCliente_pdf').on('click', function() {
    generarPDF();
  });
...
function eventoCerrarModal(){
  ...
  if($('#descripcion_Tipocliente').val() != ""){ //validad que los input hayan datos
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          $('#registral_Tipocliente').modal('show');
        }
      });
  }
 
