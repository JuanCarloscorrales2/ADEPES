//función para generar el PDF y abrirlo en una nueva ventana Del listado de Estado Solicitud
function generarPDF() {
    var datosFiltradosOrdenados = tablaEstadosolicitud.rows({ search: 'applied', order: 'applied' }).data().toArray();
    ...
    var form = $('<form action="../pages/fpdf/listadoEstadoSolicitud.php" method="post" target="_blank">');
    var input = $('<input type="hidden" name="data">').val(jsonData);
    form.append(input);
  
    // Agregar el formulario a la página y enviarlo
    form.appendTo('body').submit();
    EventoBitacora(1, 22, "Imprimió el reporte de LISTADO DE ESTADO DE SOLICITUDES");//bitacora reporte
  }
  
  //botón en la página para generar y descargar el PDF
  $('#btn_descargar_reporteEstadoSolicitud_pdf').on('click', function() {
    generarPDF();
  });
...
function eventoCerrarModal(){
  ...
  if($('#descripcion_estadosolicitud').val() != ""){ //validad que los input hayan datos
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          $('#registral_Estadosolicitud').modal('show');
        }
      });
  }
}
