//función para generar el PDF y abrirlo en una nueva ventana DEl listado de estado de credito.
function generarPDF() {
    var datosFiltradosOrdenados = tablaEstadocredito.rows({ search: 'applied', order: 'applied' }).data().toArray();
    ...
    var form = $('<form action="../pages/fpdf/listadoEstadoCredito.php" method="post" target="_blank">');
  ...
  $('#btn_descargar_rEstadoCredito_pdf').on('click', function() {
    generarPDF();
  });
...
function eventoCerrarModal(){
  ...
  if($('#descripcion_Estadocredito').val() != ""){ //validad que los input hayan datos
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          $('#registral_Estadocredito').modal('show');
        }
      });
  }
 
