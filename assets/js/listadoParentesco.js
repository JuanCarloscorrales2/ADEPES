//función para generar el PDF y abrirlo en una nueva ventana DEl listado Parentescos
function generarPDF() {
    var datosFiltradosOrdenados = tablaParentesco.rows({ search: 'applied', order: 'applied' }).data().toArray();
    ...
    var form = $('<form action="../pages/fpdf/listadoParentesco.php" method="post" target="_blank">');
  ...
  $('#btn_descargar_rParentesco_pdf').on('click', function() {
    generarPDF();
  });
...
function eventoCerrarModal(){
  ...
  if($('#descripcion_Parentesco').val() != ""){ //validad que los input hayan datos
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          $('#registral_Parentesco').modal('show');
        }
      });
  }
 
