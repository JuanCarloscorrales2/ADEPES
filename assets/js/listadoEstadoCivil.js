...
function generarPDF() {
    var datosFiltradosOrdenados = tablaEstadoCivil.rows({ search: 'applied', order: 'applied' }).data().toArray();
    ...
    var form = $('<form action="../pages/fpdf/listadoEstadoCivil.php" method="post" target="_blank">');
    var input = $('<input type="hidden" name="data">').val(jsonData);
    form.append(input);
  
    // Agregar el formulario a la página y enviarlo
    form.appendTo('body').submit();
    EventoBitacora(1, 11, "Imprimió el reporte de LISTADO DE ESTADO CIVIL");//bitacora reporte
  }
  
  //botón en la página para generar y descargar el PDF
  $('#btn_rEstCivil_pdf').on('click', function() {
    generarPDF();
  });
...
function eventoCerrarModal(){
  ...
  if($('#descripcion_estado_civil').val() != ""){ //validad que los input hayan datos
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          $('#registral_estadocivil').modal('show');
        }
      });
  }
 
