//función para generar el PDF y abrirlo en una nueva ventana del Listado de Tiempo Laboral.
function generarPDF() {
    var datosFiltradosOrdenados = tablaLaboral.rows({ search: 'applied', order: 'applied' }).data().toArray();
    ...
    var form = $('<form action="../pages/fpdf/listadoTiempoLaboral.php" method="post" target="_blank">');
    var input = $('<input type="hidden" name="data">').val(jsonData);
    form.append(input);
  
    // Agregar el formulario a la página y enviarlo
    form.appendTo('body').submit();
    EventoBitacora(1, 18, "Imprimió el reporte de LISTADO DE TIEMPO LABORAL");//bitacora reporte
  }
  
  //botón en la página para generar y descargar el PDF
  $('#btn_descargar_reporteTLaboral_pdf').on('click', function() {
    generarPDF();
  });
...
function eventoCerrarModal(){
  ...
  if($('#descripcion_Laboral').val() != ""){ //validad que los input hayan datos
