//función para generar el PDF y abrirlo en una nueva ventana Del listado de Tiempo vivir
function generarPDF() {
    var datosFiltradosOrdenados = tablaTiempovivir.rows({ search: 'applied', order: 'applied' }).data().toArray();
    ...
    var form = $('<form action="../pages/fpdf/listadoTiempoVivir.php" method="post" target="_blank">');
    var input = $('<input type="hidden" name="data">').val(jsonData);
    form.append(input);
  
    // Agregar el formulario a la página y enviarlo
    form.appendTo('body').submit();
    EventoBitacora(1, 20, "Imprimió el reporte de LISTADO DE TIEMPO DE VIVIR");//bitacora reporte
  }
  
  //botón en la página para generar y descargar el PDF
  $('#btn_descargar_reporteTiempoVivir_pdf').on('click', function() {
    generarPDF();
  });
...
function eventoCerrarModal(){
  ...
  if($('#descripcion_Tiempovivir').val() != ""){ //validad que los input hayan datos
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          $('#registral_Tiempovivir').modal('show');
        }
      });
  }
}
