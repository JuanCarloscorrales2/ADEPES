...
function generarPDF() {
    var datosFiltradosOrdenados = tablaMunicipio.rows({ search: 'applied', order: 'applied' }).data().toArray();
    ...
    var form = $('<form action="../pages/fpdf/listadoMunicipios.php" method="post" target="_blank">');
    var input = $('<input type="hidden" name="data">').val(jsonData);
    form.append(input);

    // Agregar el formulario a la página y enviarlo
    form.appendTo('body').submit();
    EventoBitacora(1, 25, "Imprimió el reporte de LISTADO DE MUNICIPIOS");//bitacora reporte
}

// Agregar un botón en la página para generar y descargar el PDF
$('#btn_descargar_reporteMunicipio_pdf').on('click', function() {
    generarPDF();
});
...
function eventoCerrarModal(){
    ...
    if($('#descripcion_Municipio').val() != ""){ //validad que los input hayan datos
      ...
      Swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            $('#registral_Municipio').modal('show');
          }
        });
    }
   
