//función para generar el PDF y abrirlo en una nueva ventana del listado Nacionalidades
function generarPDF() {
    var datosFiltradosOrdenados = tablaNacionalidad.rows({ search: 'applied', order: 'applied' }).data().toArray();
    ...
    var form = $('<form action="../pages/fpdf/listadoNacionalidad.php" method="post" target="_blank">');
    var input = $('<input type="hidden" name="data">').val(jsonData);
    form.append(input);
  
    // Agregar el formulario a la página y enviarlo
    form.appendTo('body').submit();
    EventoBitacora(1, 16, "Imprimió el reporte de LISTADO DE NACIONALIDADES");//bitacora reporte
  }
  
  //botón en la página para generar y descargar el PDF
  $('#btn_descargar_reporteNacionalidad_pdf').on('click', function() {
    generarPDF();
  });
...
function eventoCerrarModal(){
  ...
  if($('#descripcion_Nacionalidad').val() != ""){ //validad que los input hayan datos
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          $('#registral_Nacionalidad').modal('show');
        }
      });
  }
}
