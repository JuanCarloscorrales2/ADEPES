...
class PDF extends FPDF
{
   function Header() // Cabecera de página
   {
      ...
      $this->Cell(20, 10, utf8_decode("LISTADO DE ESTADO TIPO DE PRÉSTAMO"), 0, 1, 'C', 0);
      $this->Ln(7);
     ...
     $this->Cell(60, 10, utf8_decode('ESTADO TIPO DE PRÉSTAMO'), 1, 0, 'C', 1);
     $this->Ln();
   }
   // Pie de página
   function Footer(){
...
$pdf->SetDrawColor(163, 163, 163); //colorBorde 
    ...
    foreach ($datosFiltradosOrdenados as $fila) {
        //$pdf->Cell(10, 10, $i++, 1, 0, 'C', 0); //LOS NOMBRE SON DE LA FUNCION: LlenarTablaGenero JS
        $pdf->Cell(10, 10, $fila['NO'], 1, 0, 'C', 0);
        $pdf->Cell(60, 10, utf8_decode($fila['ESTADOTIPOPRESTAMO']), 1, 0, 'C', 0);
        $pdf->Ln();
    }
    // Enviar el PDF al navegador para su descarga 
    $pdf->Output('listado_de_Estado_Tipo_Prestamo.pdf', 'I'); //nombre de descarga
} else {
    echo 'error';
}
?>