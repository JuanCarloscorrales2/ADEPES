...
class PDF extends FPDF
{
   ...
   {
      $this->Image('logo.png', 165, 15, 30); //logo de la empresa,moverDerecha,moverAbajo,tamañoIMG
      ...
      $this->Cell(20, 10, utf8_decode("LISTADO DE OBJETOS"), 0, 1, 'C', 0);
      $this->Ln(7);
     ...
     $this->Cell(80, 10, utf8_decode('OBJETOS'), 1, 0, 'C', 1);
     $this->Ln();
   }
   // Pie de página
   function Footer(){
      ...
      $this->Cell(210, 10, utf8_decode('Fecha y Hora de impresión: '.$hoy), 0, 0, 'C');
   }
...
$pdf = new PDF();
$pdf->AddPage("portrait","A4"); /* aqui entran dos para parametros (horientazion,tamaño)V->portrait H->landscape tamaño (A3.A4.A5.letter.legal) */
$pdf->AliasNbPages(); //muestra la pagina / y total de paginas

$i=1;
$pdf->SetFont('Arial', '', 10);
$pdf->SetDrawColor(163, 163, 163); //colorBorde 
$pdf->setWidths(array(10, 80)); //tamano de las celdas segun el encabezado de la tabla
    //construir el contenido del PDF DE LOS DATOS OBTENIDOS DEL DATATABLESS
    foreach ($datosFiltradosOrdenados as $fila) {
        $pdf->Row(array($fila['NO'], utf8_decode($fila['OBJETO'])), 0 );
    }
    // Enviar el PDF al navegador para su descarga 
    $pdf->Output('listado_de_objetos.pdf', 'I'); //nombre de descarga
} else {
    echo 'error';
}
?>