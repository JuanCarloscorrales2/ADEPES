...
class PDF extends FPDF
{
   function Header() // Cabecera de página
   {
      ...
      $this->Cell(20, 10, utf8_decode("LISTADO DE CRÉDITO AVAL"), 0, 1, 'C', 0);
      $this->Ln(7);
     ...
     $this->Cell(60, 10, utf8_decode('CRÉDITO AVAL'), 1, 0, 'C', 1);
     $this->Ln();
   }
   // Pie de página
   function Footer(){
...
$pdf->setWidths(array(10, 60)); //tamano de las celdas segun el encabezado de la tabla
    ...
    foreach ($datosFiltradosOrdenados as $fila) {
        $pdf->Row(array($fila['NO'], utf8_decode($fila['CREDITOAVAL'])), 0 );
    }
    // Enviar el PDF al navegador para su descarga 
    $pdf->Output('listado_de_credito_aval.pdf', 'I'); //nombre de descarga
} else {
    echo 'error';
}
?>