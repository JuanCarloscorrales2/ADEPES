...
class PDF extends FPDF
{
   function Header() // Cabecera de página
   {
      ...
      $this->Cell(20, 10, utf8_decode("LISTADO DE ESTADOS DE ANÁLISIS CREDITICIO"), 0, 1, 'C', 0);
      $this->Ln(7);
     ...
     $this->Cell(75, 10, utf8_decode('ESTADO DE ANÁLISIS CREDITICIO'), 1, 0, 'C', 1);
     $this->Ln();
   }
   // Pie de página
   function Footer(){
      ...
      $this->Cell(75, 10, utf8_decode('Página ') . $this->PageNo() . '/{nb}', 0, 0, 'C'); //pie de pagina(numero de pagina)
       //fecha
       $this->SetY(-15); // Posición: a 1,5 cm del final
      $this->SetFont('Arial', 'I', 8); //tipo fuente, negrita(B-I-U-BIU), tamañoTexto
      $hoy = date('d/m/Y H:i:s');
      $this->Cell(215, 10, utf8_decode('Fecha y Hora de impresión: '.$hoy), 0, 0, 'C');
   }
...
$pdf->setWidths(array(10, 75)); //tamano de las celdas segun el encabezado de la tabla

    //construir el contenido del PDF DE LOS DATOS OBTENIDOS DEL DATATABLESS
    foreach ($datosFiltradosOrdenados as $fila) {
        $pdf->Row(array($fila['NO'], utf8_decode($fila['ANALISIS'])), 0 );
    }
    // Enviar el PDF al navegador para su descarga 
    $pdf->Output('listado_de_Estado_Analisis_Crediticio.pdf', 'I'); //nombre de descarga
} else {
    echo 'error';
}
?>