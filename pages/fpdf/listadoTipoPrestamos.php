...
class PDF extends FPDF
{
   function Header() // Cabecera de página
   {
     ...
     $this->Cell(32, 10, utf8_decode('PLAZO MÁXIMO'), 1, 0, 'C', 1);
     $this->Cell(32, 10, utf8_decode('MONTO MÍNIMO'), 1, 0, 'C', 1);
     $this->Cell(32, 10, utf8_decode('MONTO MÁXIMO'), 1, 0, 'C', 1);
   ...
   function Footer(){
      ...
      $this->Cell(180, 10, utf8_decode('Página ') . $this->PageNo() . '/{nb}', 0, 0, 'C'); //pie de pagina(numero de pagina)
       //fecha
       $this->SetY(-15); // Posición: a 1,5 cm del final
      $this->SetFont('Arial', 'I', 8); //tipo fuente, negrita(B-I-U-BIU), tamañoTexto
      $hoy = date('d/m/Y H:i:s');
      $this->Cell(315, 10, utf8_decode('Fecha y Hora de impresión: '.$hoy), 0, 0, 'C');
   }
...
$pdf->setWidths(array(15, 30, 30, 20, 32, 32, 32)); //tamano de las celdas segun el encabezado de la tabla
    //construir el contenido del PDF DE LOS DATOS OBTENIDOS DEL DATATABLESS
    foreach ($datosFiltradosOrdenados as $fila) {
        $pdf->Row(array($fila['NO'], $fila['ESTADO'], $fila['DESCRIPCION'], $fila['TASA'], $fila['PLAZO'], $fila['MINIMO'], $fila['MAXIMO']), 0);
    }
