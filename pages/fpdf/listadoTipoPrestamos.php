...
class PDF extends FPDF
{
   function Header() // Cabecera de página
   {
      ...
      $this->Cell(20, 10, utf8_decode("LISTADO DE TIPO DE PRÉSTAMOS"), 0, 1, 'C', 0);
      $this->Ln(7);
     ...
     $this->Cell(30, 10, utf8_decode('PRÉSTAMO'), 1, 0, 'C', 1);
     $this->Cell(20, 10, utf8_decode('TASA'), 1, 0, 'C', 1);
     $this->Cell(30, 10, utf8_decode('PLAZO MÁXIMO'), 1, 0, 'C', 1);
     $this->Cell(30, 10, utf8_decode('MONTO MÍNIMO'), 1, 0, 'C', 1);
     $this->Cell(30, 10, utf8_decode('MONTO MÁXIMO'), 1, 0, 'C', 1);
   ...
   function Footer(){
      ...
      $this->Cell(195, 10, utf8_decode('Página ') . $this->PageNo() . '/{nb}', 0, 0, 'C'); //pie de pagina(numero de pagina)
       //fecha
       $this->SetY(-15); // Posición: a 1,5 cm del final
      $this->SetFont('Arial', 'I', 8); //tipo fuente, negrita(B-I-U-BIU), tamañoTexto
      $hoy = date('d/m/Y H:i:s');
      $this->Cell(385, 10, utf8_decode('Fecha y Hora de impresión:'.$hoy), 0, 0, 'C');
   }
}
...
$pdf->SetDrawColor(163, 163, 163); //colorBorde 
    ...
    foreach ($datosFiltradosOrdenados as $fila) {
        ...
        $pdf->Cell(15, 10, $fila['NO'], 1, 0, 'C', 0);
        $pdf->Cell(30, 10, $fila['ESTADO'], 1, 0, 'C', 0);
        $pdf->Cell(30, 10, $fila['DESCRIPCION'], 1, 0, 'C', 0);
        $pdf->Cell(20, 10, $fila['TASA'], 1, 0, 'C', 0);
        $pdf->Cell(30, 10, $fila['PLAZO'], 1, 0, 'C', 0);
        $pdf->Cell(30, 10, $fila['MINIMO'], 1, 0, 'C', 0);
        $pdf->Cell(30, 10, $fila['MAXIMO'], 1, 0, 'C', 0);
        $pdf->Ln();
    }
    // Enviar el PDF al navegador para su descarga 
    $pdf->Output('listado_de_TiposPrestamos.pdf', 'I'); //nombre de descarga
} else {
    echo 'error';
}
?>