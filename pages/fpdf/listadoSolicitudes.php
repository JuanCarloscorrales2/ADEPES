...
class PDF extends FPDF
{
   ...
   function Header()
   {
      ...
      $this->Cell(35, 10, utf8_decode("LISTADO DE SOLICITUDES"), 0, 1, 'C', 0);
      $this->Ln(7);
     ...
     $this->Cell(75, 10, utf8_decode('SOLICITANTE'), 1, 0, 'C', 1);
     $this->Cell(32, 10, utf8_decode('GARANTÍA'), 1, 0, 'C', 1);
     $this->Cell(35, 10, utf8_decode('MONTO'), 1, 0, 'C', 1);
     $this->Cell(22, 10, utf8_decode('TASA'), 1, 0, 'C', 1);
...
$pdf->SetWidths(array(12, 75, 32, 35, 22, 20, 30, 30));    
//construir el contenido del PDF DE LOS DATOS OBTENIDOS DEL DATATABLESS
foreach ($datosFiltradosOrdenados as $fila) {
    
    $pdf->Row(array( $fila['IdSe'],$fila['NOMBRE'],$fila['PRESTAMO'],$fila['MONTO'], $fila['TASA'], $fila['PLAZO'], $fila['ESTADON'],$fila['FECHA_DESEMBOLSO']), 0 );
    
}


    // Enviar el PDF al navegador para su descarga 
    $pdf->Output('listado_de_solicitudes.pdf', 'I'); //nombre de descarga
} else {
    echo 'error';
}
?>
