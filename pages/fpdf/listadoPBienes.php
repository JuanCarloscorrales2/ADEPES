...
class PDF extends FPDF
{
   function Header() // Cabecera de página
   {
      ...
      $this->Cell(20, 10, utf8_decode("LISTADO DE PERSONA CON BIENES"), 0, 1, 'C', 0);
      $this->Ln(7);
     ...
     $this->Cell(60, 10, utf8_decode('PERSONAS BIENES'), 1, 0, 'C', 1);
     $this->Ln();
   }
   // Pie de página
   function Footer(){
...
$pdf = new PDF();
$pdf->AddPage("potrait","letter"); /* aqui entran dos para parametros (horientazion,tamaño)V->portrait H->landscape tamaño (A3.A4.A5.letter.legal) */
...
$pdf->setWidths(array(10, 60)); //tamano de las celdas segun el encabezado de la tabla
    ...
    foreach ($datosFiltradosOrdenados as $fila) {
        $pdf->Row(array($fila['NO'], utf8_decode($fila['BIENES'])), 0 );
    }
    // Enviar el PDF al navegador para su descarga 
    $pdf->Output('listado_de_Persona_con_Bienes.pdf', 'I'); //nombre de descarga
} else {
    echo 'error';
}
?>